...
ATLAS uses a right-handed coordinate system, with the \gls{IP} defining the origin. The $z$-axis is along the beamline, the $x$-axis points toward the center of the \gls{LHC}, and the $y$-axis points upwards, toward Earth's surface.  Transverse quantities such as \pt and \et are calculated in the $x-y$ plane. The half of the detector pointed toward the Saleve is known as the ``A-side'' and has positive $z$ coordinates. The side with negative $z$, pointed toward the Jura is denoted ``C-side,'' where ``B'' is the barrel.

In terms of cylindrical coordinates, $\phi$, the azimuthal angle, is measured around the beam axis and $\theta$, the polar angle, is the angle from the beam axis. Commonly, the \textit{rapidity} is used,

\begin{equation}
    y = \frac{1}{2} \log{\frac{E+p_z}{E-p_z}}
\end{equation}

The \textit{pseudorapidity}, defined as 

\begin{equation}
    \eta = -\ln[\tan(\theta / 2)],
\end{equation}

is Lorentz invariant under longitudinal boosts, and used to define the distance between particles as
...
The straws are filled with a gaseous mixture composed of 70\% $Xe$, 27\% $CO_2$, and 3\% $O_2$. The radiator between photons is made of polypropylene, which, having a different dielectric constant than the gaseous mixture, can produce transition radiation photons. These photons can ionize the xenon gas, which creates the signal pulse.
...
The \glsfirst{MS} \cite{muon-tdr} is the outermost layer of the ATLAS detector, aiming to track muons via independent triggering (up to $|\eta| < 2.4$) and momentum measurement (up to $|\eta| < 2.7$). Triggers conducted in the \gls{MS} are combined with information from the \gls{ID} to make the final trigger decision, so good timing resolution is imperative.
...
\begin{figure}[!ht]
    ...
    \caption[A flowchart indicating how trigger decisions are executed with rates labeled]{A flowchart indicating how trigger decisions are executed with rates labeled \cite{trigger2010}.}
